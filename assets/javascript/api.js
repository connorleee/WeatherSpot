...
var api = [
  {
    ...
    weatherApi: function () {
      ...
      $("#buttonID").on("click", function getLocation() {
        ...
        elem.innerHTML =
          '<img class="background" src="assets/images/seattle.jpg">';
        if (navigator.geolocation) {
          ...
          navigator.geolocation.getCurrentPosition(
            ...
            function success(position) {
              ...
              function displayWeatherForecastByGeolocation() {
                var queryURLGeolocationForWeatherForecase =
            ...
            function error(error_message) {
              ...
              console.error(
                "An error has occured while retrieving loction",
                error_message
              );
            }
          );
          ...
          alert(
            "Geolocation is not supported by this browser, please input your city name in the search box!"
          );
        }
      });
      ...
      function displayWeatherData(weather) {
        ...
        var cityAndCountry =
          cityNameByGeolocation + ", " + countryNameByGeolocation;
        ...
        function determineWeatherAnimation() {
          var elem = document.querySelector("#background");
          elem.innerHTML =
            '<img class="background" src="assets/images/' +
            weatherMain +
            '.jpg" alt="' +
            weatherMain +
            '">';
      ...
      function timeZone(weather) {
        ...
        $.ajax({
          url:
            ...
            "," +
            lon +
            "&key=AIzaSyAz5PoWXceWmVdDEsy4odPgaJvCZMO9jLY&timestamp=" +
      ...
      function displayWeatherForecastData(weatherForecast) {
        ...
        for (var d = 0; d < 5; d++) {
          if (d == 0) {
            startIndex = 0;
            forecastDates = moment(weatherForecast.list[0].dt, "X").format(
              "ddd MMM Do"
            );
            ...
            forecastDates = moment(
              weatherForecast.list[startIndex].dt,
              "X"
            ).format("ddd MMM Do");
            forecasthour = moment(
              weatherForecast.list[startIndex].dt,
          ...
          for (var i = startIndex; i < startIndex + weatherLength; i++) {
            ...
            var weatherIconIDForFuture =
              weatherForecast.list[i].weather[0].icon;
            var weatherIconURLForFuture =
              "https://openweathermap.org/img/w/" +
              weatherIconIDForFuture +
              ".png";
            ...
            $("#body" + d).append(
              `<p id = "weather-forecast-p", style="color:blue;"> ${hour} ${temp} ${description}</p>`
            );
          }
        }
      }
      ...
      function displayWeatherForecastByCity() {
        ...
        var cityName = $(".location").val().trim();
        var APIKey = "78c022ae7b87430bbaabb56f3fd651a0";
        var queryURLCityForWeatherForecast =
  ...
  // Youtube API Call
  {
    youtubeApi: function (weatherMainReturn) {
      ...
      var pidDrizzle =
        "PLuXiwKradYWMuaTv2KlL134p4hqiDjIl3"; /* Acoustic Guitar Instrumentals */
      var pidClouds = "PLKYTmz7SemaqVDF6XJ15bv_8-j7ckkNgb"; /* lo-fi hip hop */
      var pidSnow =
        "PLBiHiGDiFvmjV_hVEQkFUV2WmD5ubKXxP"; /* Music-Warm Winter Season */
      var pidThunder =
        "PLsj2E7daicxYNzGv_aFjKhzTsb4wtviz9"; /* Thunderstruck \m/ */
      var pidRain = "PLJzjrheyqoDVnOXyOCluGuBtV0K-8seCC"; /* Rainy Day */
      var pidClear =
        "PLHOyawPtVknXCyiXycVftCM-8LOICtBp6"; /* Have a great day */

      var currentWeatherPlaylist;
      // conditionals to insert current weather into query
      if (weatherMainReturn === "rain") {
        currentWeatherPlaylist = pidRain;
      } else if (weatherMainReturn === "clear") {
        currentWeatherPlaylist = pidClear;
      } else if (weatherMainReturn === "thunderstorm") {
        currentWeatherPlaylist = pidThunder;
      } else if (weatherMainReturn === "drizzle") {
        currentWeatherPlaylist = pidDrizzle;
      } else if (weatherMainReturn === "clouds") {
        currentWeatherPlaylist = pidClouds;
      } else if (weatherMainReturn === "snow") {
        currentWeatherPlaylist = pidSnow;
      }
      ...
      $.ajax({
        ...
        for (let i = 0; i < 5; i++) {
          var playlistTitle = playlist[i].snippet.title
            .split(" ")
            .slice(0, 5)
            .join(" ");
          var listItems = $("<button>")
            .addClass("playlistTitle btn-hover")
            .attr("videoID", playlist[i].snippet.resourceId.videoId)
  ...
  //   flickr API call for photos
  {
    flickrApi: function () {
      const apiKey = "3ae07663fb41077c9e2d01dd161feeb2";
      const apiSecret = "14619590de0c2124";
      let method = "flickr.photos.search";
      let lat = "47.624134";
      let lon = "-122.313507";

      let queryUrl = `https://www.flickr.com/services/rest/?method=${method}&api_key=${apiKey}&lat=${lat}&lon=${lon}`;
